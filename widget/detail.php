...
<?php
    ...
    if (isset($_GET['id'])) {
        $gameid = $_GET['id'];
        $searchName = mysqli_query($conn, "SELECT gamename FROM listgame WHERE gameid = '$gameid'");
        ...
        <div class="container px-5 game-detail">
            <div class="game-detail">
                <?php
                    if (isset($_GET['id'])) {
                        $gameid = $_GET['id'];
                        $gameDetail = mysqli_query($conn, "SELECT * FROM listgame WHERE gameid = '$gameid'");
                        if ($row = mysqli_fetch_array($gameDetail)) {
                            ...
                            if((isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true)) {
                                ...
                                if ($price == 0 && $gametag != 'upcoming') {
                                    $buybtn = '<a href="/projectEG-php/widget/login.php" class="btn btn-primary game-aside__btn">GET</a>';
                                    $pricebar = '
                                        <div class="game-aside__order">
                                            <p class="title game-aside__price">Free</p>
                                        </div>
                                    ...
                                    $buybtn = '<a href="/projectEG-php/widget/login.php" class="btn btn-outline-light game-aside__btn" disabled>COMING SOON</a>';
                                    $pricebar = '
                                        <div class="game-aside__order">
                                            <p class="title game-aside__price">Free</p>
                                        </div>
                                    ...
                                    $buybtn = '<a href="/projectEG-php/widget/login.php" class="btn btn-primary game-aside__btn">BUY NOW</a>';
                                    $pricebar = '
                                        <div class="game-aside__order">
                                            <p class="title game-aside__price">đ'.number_format($price).'</p>
                                        </div>
                            ...
                            if(isset($_SESSION['email'])) {
                                ...
                                if (mysqli_num_rows($check) > 0) {
                                    $addBtn = '
                                        <a href="./cart.php" class="btn btn-outline-light game-aside__btn game-aside__btn--add">VIEW IN CART</a>
                                    ';
                                } else {
                                    $addBtn = '
                                ...
                                $addBtn = '
                                    <a href="./cart.php" class="btn btn-outline-light game-aside__btn game-aside__btn--add">ADD TO CART</a>
                                ';
                            }
                            
                            // Find genre name 
                            $genreResult = mysqli_query($conn, "SELECT * FROM genrelist WHERE genreid = '$genreid'");
...
<script>
    ...
    if (addBtn.getAttribute("gameid") != null) {
        addBtn.addEventListener('click', function() {
            ...
            window.location="/projectEG-php/assets/php/addToCart.php?id=" + gameid
        })
    }
</script>
</html>
